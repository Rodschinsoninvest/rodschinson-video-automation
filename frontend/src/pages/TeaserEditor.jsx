                      ...
                      {photos.map((url, pi) => (
                        <div key={`${url}-${pi}`} style={{ position: 'relative', borderRadius: 6, overflow: 'hidden', border: `1px solid ${border}`, background: panel }}>
                          ...
                          <div style={{ position: 'absolute', top: 4, right: 4, display: 'flex', gap: 4 }}>
                            {pi > 0 && <button onClick={() => { const arr = [...photos];[arr[pi - 1], arr[pi]] = [arr[pi], arr[pi - 1]]; setAssetField(idx, 'photos', arr) }} title="Move up" style={{ width: 22, height: 22, borderRadius: 3, border: 'none', background: 'rgba(0,0,0,0.6)', color: '#fff', cursor: 'pointer', fontSize: 11 }}>↑</button>}
